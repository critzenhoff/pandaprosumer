...
def _calculate_electric_boiler_temp(mdot_kg_per_s, t_out_c, t_in_c, cp_fluid_kj_per_kgk, efficiency_percent, max_p_kw):
    # CARL bis 15
    if t_out_c - t_in_c < 0.01:
        return 0.0, mdot_kg_per_s, t_in_c, t_in_c, 0.0
    ...
    if max_p_kw is not None and p_el_consumed_kw > max_p_kw + 1e-3:
        # Downstream-geforderten Massenstrom NICHT reduzieren - stattdessen
        # die Austrittstemperatur an die verfügbare (elektrische) Leistung anpassen
        p_el_consumed_kw = max_p_kw
        q_fluid_kw = p_el_consumed_kw * (efficiency_percent / 100)
        if mdot_kg_per_s > 1e-9:
...
class ElectricBoilerController(BasicProsumerController):
    ...
    def _t_in_c(self):
        """
        Reale physische Eingangstemperatur von einem upstream FluidMix-Initiator
        (z.B. SHS-Discharge). NaN, falls kein Producer gemappt ist (Standalone-Fall).
        """
    ...
    def t_m_to_deliver(self, prosumer):
        t_out_c, t_in_c, mdot_tab_kg_per_s = super().t_m_to_deliver(prosumer)
        t_target_c = self._get_element_param(prosumer, 'max_t_out_c')
        if t_target_c is not None and not np.isnan(t_target_c):
            t_out_c = t_target_c
        return t_out_c, t_in_c, mdot_tab_kg_per_s
    ...
    def control_step(self, prosumer):
        ...
        t_in_c_actual = self._t_in_c #Carl +3
        if np.isnan(t_in_c_actual):
            t_in_c_actual = t_in_required_c  # Fallback: Standalone ohne Upstream-Mapping
        ...
        assert not np.isnan(
            t_in_c_actual), f"Electric Boiler {self.name} t_in_c_actual is NaN for timestep {self.time} in prosumer {prosumer.name}" #CARL
