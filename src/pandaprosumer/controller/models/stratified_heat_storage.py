...
### ------------------------------------------------------------------------- CARL Beginn
    def get_net_demand(self, prosumer):
        """
        Real, current net demand behind the storage (via its mapped
        responder, e.g. the network coupling controller). Uses the
        UNCHANGED, inherited t_m_to_deliver - this is the actual downstream
        request, just exposed in a convenient (kW, degC, degC, kg/s) format.

        :return: (q_demand_kw, t_feed_c, t_return_c, mdot_kg_per_s)
        """
        t_demand_out_c, t_demand_in_c, mdot_demand_tab_kg_per_s = self.t_m_to_deliver(prosumer)
        mdot_demand_kg_per_s = float(np.sum(mdot_demand_tab_kg_per_s))
        if mdot_demand_kg_per_s > 1e-8:
            cp_j_per_kgk = float(self.fluid.get_heat_capacity(CELSIUS_TO_K + (t_demand_out_c + t_demand_in_c) / 2))
            q_demand_kw = mdot_demand_kg_per_s * cp_j_per_kgk * (t_demand_out_c - t_demand_in_c) / 1e3
        else:
            q_demand_kw = 0.0
        return q_demand_kw, t_demand_out_c, t_demand_in_c, mdot_demand_kg_per_s

    def is_full(self, prosumer, tol_c=None):
        """
        True if there is no useful cold layer left to charge, i.e. the
        coldest layer has reached the charge target. This is the direct,
        robust check for the charge/discharge decision (avoids the sign
        ambiguity of get_soc below).
        """
        if tol_c is None:
            tol_c = self._get_element_param(prosumer, 't_discharge_out_tol_c')
        t_charge_target_c = self._get_element_param(prosumer, 'min_useful_temp_c')
        return bool(np.min(self._layer_temps_c) >= t_charge_target_c - tol_c)

    def get_soc(self, prosumer):
        """
        Continuous 0..1-ish fill indicator, intended for LATER use (price-
        based modulation, etc.).

        CAVEAT (unverified/approximate): the underlying energy formulas in
        this model are defined RELATIVE to init_layer_temps_c (see the
        paper's eq. 8), not as an absolute reference. If min_useful_temp_c
        is lower than init_layer_temps_c, the ratio below can behave
        counter-intuitively (e.g. go negative or exceed 1). For the current
        binary charge/discharge decision, use is_full() instead - only rely
        on get_soc() for future continuous-control logic after verifying it
        against your actual init_layer_temps_c / min_useful_temp_c choice.
        """
        min_useful_temp_c = self._get_element_param(prosumer, 'min_useful_temp_c')
        e_now_kwh = self._get_stored_energy_kwh(min_useful_temp_c)
        e_max_kwh = self._get_max_stored_energy_kwh(min_useful_temp_c, min_useful_temp_c)
        return e_now_kwh / e_max_kwh if abs(e_max_kwh) > 1e-6 else 0.0
    ...
    def _t_m_to_receive_init(self, prosumer):
        ...
        # # StratifiedHeatStorageController._t_m_to_receive_init, in stratified_heat_storage.py:
        # if not np.isnan(self.t_previous_out_charge_c): # CARL bis 364
        #     # Bekannten, tatsächlich erhaltenen Zustand wiederverwenden statt
        #     # instabile Neuberechnung (verhindert die mdot-Oszillation)
    ...
    def _get_max_stored_energy_kwh(self, extraction_temp_c, t_charge_c):
        ...
        ret = sum([float(self.fluid.get_density(CELSIUS_TO_K + t_charge_c)) * self.A_m2 *
                   ...
                   (t_charge_c - t_ext_c) * self.dz_m #self.init_layer_temps_c[z]
    ...
    def _get_stored_energy_kwh(self, t_extraction_c):
        ...
        """
        t_ext_c = 20 #self._get_element_param(prosumer, 't_ext_c')
        # 3.6e6 is the conversion factor from J to kWh
        ret = sum([float(self.fluid.get_density(CELSIUS_TO_K + self._layer_temps_c[z])) * self.A_m2 *
                   ...
                   (self._layer_temps_c[z] - t_ext_c) * self.dz_m  # self.init_layer_temps_c[z] CARLLL
    ...
    def _calculate_heat_storage(self, prosumer, mdot_demand_kg_per_s, t_received_in_c, t_demand_out_c, t_demand_in_c,
        ...
        if not self.bypass:
            ...
            # cp_check_j_per_kgk = float(self.fluid.get_heat_capacity(CELSIUS_TO_K + t_received_in_c))  # Carl bis 429
            # q_received_available_kw = mdot_received_kg_per_s * cp_check_j_per_kgk * (
            #             t_received_in_c - t_demand_in_c) / 1e3
            # q_demand_kw = mdot_demand_kg_per_s * cp_check_j_per_kgk * (t_demand_out_c - t_demand_in_c) / 1e3
            #
            # if q_received_available_kw < q_demand_kw - 1e-6:
            #     # Leistungslücke: HP liefert energetisch zu wenig -> Discharge aktivieren
            #     mdot_bypass_kg_per_s = mdot_received_kg_per_s
            #     t_bypass_in_c = t_received_in_c
            #     mdot_charge_kg_per_s = 0
            #     q_missing_kw = q_demand_kw - q_received_available_kw
            #     cp_discharge_check = float(
            #         self.fluid.get_heat_capacity(CELSIUS_TO_K + (t_discharge_out_c + t_demand_in_c) / 2))
            #     mdot_discharge_kg_per_s = q_missing_kw * 1e3 / (
            #                 cp_discharge_check * max(t_discharge_out_c - t_demand_in_c, 1e-6))
            # else:
    ...
    def control_step(self, prosumer):
        ...
        # if np.isnan(self.t_keep_return_c) or mdot_received_kg_per_s == 0 or abs(t_received_out_c - self.t_keep_return_c) < TEMPERATURE_CONVERGENCE_THRESHOLD_C or len(self._get_mapped_initiators_on_same_level(prosumer)) == 0:
            # NACHHER:
        if np.isnan(self.t_keep_return_c) or mdot_received_kg_per_s == 0: # CARL bis 679
            energy_converged = True
        else:
            cp_check_j_per_kgk = float(self.fluid.get_heat_capacity(CELSIUS_TO_K + t_received_in_c))
            q_mismatch_kw = abs(mdot_received_kg_per_s * cp_check_j_per_kgk
                                * (t_received_out_c - self.t_keep_return_c) / 1e3)
            # Relative Toleranz (0.5% von q_received) mit Mindestwert 1 kW, um Div/0 bei kleinen Leistungen zu vermeiden
            energy_tolerance_kw = max(1.0, 0.005 * abs(q_received_kw))
            energy_converged = q_mismatch_kw < energy_tolerance_kw

        if np.isnan(self.t_keep_return_c) or mdot_received_kg_per_s == 0 or energy_converged or len(
                self._get_mapped_initiators_on_same_level(prosumer)) == 0:
