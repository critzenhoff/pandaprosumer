...
class HeatPumpController(BasicProsumerController):
    ...
    def control_step(self, prosumer):
        ...
        # if np.isnan(self.t_keep_return_c) or mdot_evap_kg_per_s == 0 or abs(t_evap_out_c - self.t_keep_return_c) < TEMPERATURE_CONVERGENCE_THRESHOLD_C or len(self._get_mapped_initiators_on_same_level(prosumer)) == 0:
            # NACHHER:
        if np.isnan(self.t_keep_return_c) or mdot_evap_kg_per_s == 0: # CARL
            energy_converged = True
        else:
            cp_evap_check_kj_per_kgk = self.evap_fluid.get_heat_capacity(CELSIUS_TO_K + t_evap_in_c) / 1000
            q_mismatch_kw = abs(
                mdot_evap_kg_per_s * cp_evap_check_kj_per_kgk * (t_evap_out_c - self.t_keep_return_c))
            energy_tolerance_kw = max(1.0, 0.005 * abs(q_evap_kw))
            energy_converged = q_mismatch_kw < energy_tolerance_kw

        if np.isnan(self.t_keep_return_c) or mdot_evap_kg_per_s == 0 or energy_converged or len(
